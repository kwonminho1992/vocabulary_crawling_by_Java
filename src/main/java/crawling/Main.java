...
public class Main {
	...
	/**
	 * Proceed crawling
	 * If you want to run English, input "english" into parameter. (in case Vietnamese, input "vietnamese")
	 * @param String choice
	 * @throws InterruptedException
	 */
	public static void runCrawling(String choice) throws InterruptedException {
		Language language; // declare frame object (superclass of each languages such as English, Vietnamese...)
		if (choice.equals("english")) { // if user choose english, frame object will be English type and run English crawling 
		...
		ArrayList<ArrayList<String>> words = language.sortWords(choice); // arraylist for the words' information (part, meaning etc...)
		int size = words.size();
		for (int i = 0; i < size; i++) {
			...
			if (choice.equals("english")) { //setPartForms() method is only for English type (Other languages don't use)
			...
			if (choice.equals("english")) { //phonetic_alphabet field is only for English type
				try { //add phonetic_alphabet field
					String phoneticAlphabet = language.addPhoneticAlphabet(words.get(i).get(0), driver);
				...
				try { //add hanja field
					String hanja = language.addHanja(words.get(i).get(0), driver);
			...
			String pronunciation = language.addPronunciation(words.get(i).get(0)); // add pronunciation field
